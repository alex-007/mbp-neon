...
chroot "${CHROOT_PATH}" /bin/bash -c "KERNEL_VERSION=${KERNEL_VERSION} KVER=${KVER} PREL=${PREL} /tmp/setup_files/chroot_build.sh"
...
cp -rfv "${ROOT_PATH}/files/suspend/suspend-modules" "${CHROOT_PATH}/lib/systemd/system-sleep/suspend-modules"
cp -rfv "${ROOT_PATH}/files/suspend/suspend-modules.conf" "${CHROOT_PATH}/etc/suspend-modules.conf"
chmod a+x "${CHROOT_PATH}/lib/systemd/system-sleep/suspend-modules"
